...
Use `download_dewey_duck()` to download the dataset as parquet files to a local directory.
...
** You can specify the folder to download into: **
```[r}
base_dir = "C:/Users/mecha/outsidedrive/488_dewey_test/dewey-downloads"

download_dewey_duck(api_key, data_id, partition = "MONTH_DATE")
```
...
```r
download_dewey_duck(api_key, data_id, partition = "MONTH_DATE", overwrite = TRUE) 
```

`download_dewey_duck()` returns the path to the downloaded folder invisibly, so you can pipe directly into `read_dewey()`.

---

### 3. Read the data

Use `read_dewey_duck()` to read a previously downloaded dataset back into R as a tibble:

```r
df <- read_dewey_duck("C:/your/path/to/dewey-downloads/airline-employment")
df
...
```r
df <- download_dewey_duck(api_key, data_id, partition = "MONTH_DATE", overwrite=TRUE) |>
  read_dewey_duck()
```
...
- `read_dewey_duck()` does not require an API key — once downloaded, the data is yours
- Passing an existing folder path to `download_dewey_duck()` will error unless `overwrite = TRUE` is set
